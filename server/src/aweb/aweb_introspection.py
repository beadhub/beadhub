...
from .auth import DatabaseLike, parse_bearer_token, verify_bearer_token_details
...
async def get_identity_from_auth(request: Request, db: DatabaseLike) -> AuthIdentity:
    """Resolve the authenticated identity context for coordination requests.

    Priority order:
    1) Trusted proxy/wrapper auth context (`X-AWEB-Auth` + `X-Project-ID`)
    2) Local aweb Bearer API key mounted by the coordination core
    """
...
async def get_project_from_auth(request: Request, db: DatabaseLike) -> str:
    """Resolve the authenticated project_id for coordination requests.

    Priority order:
    1) Trusted proxy/wrapper auth context (`X-AWEB-Auth` + `X-Project-ID`)
    2) Local aweb auth mounted by the coordination core
    """
